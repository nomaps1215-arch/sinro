...
**このフォルダは OneDrive 配下にあり、同期で `data/schools.json` が
古い状態へ巻き戻ることが実際に起きた**（学校の重複が復活し、登録した入学金が消えた）。
取り込みツールを流したら、続けて build_bundle → qa_check → commit までやりきること。
git に入れておけば戻せる。

巻き戻りに気づくには、カバー数を数えるのが早い。

```bash
python tools/qa_check.py   # 末尾に未取得の件数が出る
```
