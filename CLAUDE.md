...
鉄道運賃には機械で読める公開データが無い。各事業者は検索フォームしか出しておらず、
駅間の運賃表を公開していない。駅すぱあとAPIには無料枠があるが、静的サイトに鍵を
置くことになるので使えない。距離からの推定は、営業キロの誤差（このエンジンは
直線距離×1.10）と通学定期の割引率の違いが二重に乗って数千円ずれる。

3年間で十数万円変わる数字なので、推測値は出さない。
カードの「定期代」ボタンから、乗車駅と学校の最寄り駅を入れた状態で
Yahoo!路線情報の検索へ飛ばしている。金額を出したくなったら、鍵を隠せる中継
サーバーが要る（静的サイトのままでは不可能）。ここは調査済みなので蒸し返さないこと。
