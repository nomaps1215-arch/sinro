...
曜日を必須にしているのは、「2026/09/11 ○○説明会のお知らせ」のような
ニュースの投稿日を行事の日として拾わないため。学校が告知する行事の日付には
ほぼ必ず曜日が添えてあり、曜日を必須にしても失うのは全体の3%程度だった。
...
def resolve_year(month: int, day: int, weekday: str | None,
    ...
    if not weekday:
        # 曜日の記載が無い日付は採らない。
        # 「10月11日」だけでは今年か去年の残骸か区別できないうえ、
        # 「2026/09/11 ○○説明会のお知らせ」のようなニュースの投稿日が
        # 年つきで大量に紛れ込む。実際に8校でこれを行事日として登録していた。
        # 学校が告知する行事の日付には、ほぼ必ず曜日が添えてある
        # （全件の97%に曜日があった）ので、失うものは小さい。
        return None
...
def find_events(text: str, source: str, today: dt.date) -> list[dict]:
    ...
    def candidates_for(ks: int, ke: int, check_own: bool = True) -> list:
        ...
        for i, c in enumerate(live):
            ...
            # 向きを多数決で決めてページごとに片側だけ見る案は、実物で外した。
            # 清風のページは節によって並びが逆で、多数決の側だけを見ると
            # 「プレテスト 9月11日(金)・12日(土) 文化祭」の9月11日が残った。
            # 間違った日付を出すよりは取りこぼす方がましなので、両側を見る。
            own = [text[max(0, c["start"] - ADJACENT): c["start"]],
                   text[c["end"]: c["end"] + ADJACENT]]
