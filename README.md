...
| 項目 | 収録数 | 出どころ |
|---|---|---|
| 高校 | **255校**（公立146・私立109） | 大阪府／私学連合会の公式一覧 |
| 路線・駅 | **51路線 962駅**（大阪府全域） | OpenStreetMap |
| 偏差値 | 223校（すべて「想定値」） | 民間サイトの掲載値 |
| 入試倍率・定員割れ | 122校 | 大阪府公表の実数 |
| 男女比 | 141校 | 大阪府公表の実数 |
| 入学金 | 146校（公立のみ） | 大阪府公表の額 |
...
- 校名の横の日付 … 次のオープンスクール・説明会（日程を拾えた学校だけ）
- カードをタップ … 学科別の偏差値と判定、オープンスクールの日程と申込要否、特徴のまとめ
- 「申込」ボタン … 公式サイトに申込フォームへのリンクがあった学校だけ出る
...
`data/bundle.js` の `?v=` だけは `build_bundle.py` が中身のハッシュに自動で書き換えるので、
データの更新については触らなくてよい。
...
| オープンスクールの日程・申込先 | 公式サイトからの自動取得（12時間ごと） | 掲載の書き方次第。曜日が合わない日付は捨てている |
| 私立の入学金・授業料 | [大阪私立中学校高等学校連合会「納付金等調」](https://www.osaka-shigaku.gr.jp/news/) | 一次ソース。高い（毎年11月調べ） |
| 学校の写真 | [Wikimedia Commons](https://commons.wikimedia.org/)（自由ライセンス画像のみ） | 出典・撮影者・ライセンスを表示 |
...
python tools/fetch_private_fee_pdf.py --apply
python tools/build_bundle.py
```

大阪私立中学校高等学校連合会が毎年11月に「私立高等学校新入生徒 納付金等調」を
PDFで公表している。学校別に入学金等・授業料等が載った一次資料。
`tools/pdf_text.py` が埋め込みフォントのPDFから座標つきで文字を取り出す
（標準ライブラリのみ。表を読むためだけの最小実装で、汎用のPDFライブラリではない）。

通信制・単位制は別のPDFに載っていて授業料が1単位あたりなので、入学金だけ取り込む。

以前の `tools/fetch_private_fee.py`（各校サイトを巡回する版）は使わない。
募集要項がPDFや画像の学校が多く確実に読めないうえ、こちらの方が出典として確か。

### 学校の写真を更新する

```bash
python tools/fetch_photos.py            # 調べてレポートを出すだけ
python tools/fetch_photos.py --apply
python tools/build_bundle.py
```

日本語版Wikipediaの各校の記事に使われている画像のうち、Wikimedia Commons にあり
自由ライセンスのものだけを最大4枚まで採る。**学校の公式サイトの写真は使わない**
（学校の著作物で、並べて出すのは再配布にあたる）。

拡大表示に撮影者・ライセンス・Commonsへのリンクを出している。これは自由ライセンスの
条件なので消さないこと。

MediaWiki API の `imlimit` は「1ページあたり」ではなく「問い合わせ全体」に効く。
`continue` を辿らないと、まとめて聞いた学校のほとんどで画像が空になる
（実際にこれで47校しか取れていなかった。直して233校になった）。

### オープンスクールの日程を更新する

```bash
python tools/fetch_open_school.py            # 巡回してレポートを出すだけ
python tools/fetch_open_school.py --apply    # schools.json に反映（255校で約2時間）
python tools/build_bundle.py
```

**通常は手で流す必要はない。** GitHub Actions（`.github/workflows/open-school.yml`）が
日本時間の9時と21時に自動で巡回し、差分があれば main にコミットして push する。
すぐ走らせたいときは GitHub の Actions タブ →「オープンスクール情報の巡回」→ Run workflow。

日付の扱いは慎重にしてある。受験生が予定を組むのに使う情報なので、
**曜日の記載と計算した曜日が食い違う日付は捨てる**（書式の読み違い）。
「10月11日」のように曜日も年も書かれていない日付も、今年のものか去年の残骸か
区別できないので捨てる。拾えた日付だけを出し、拾えなければ何も出さない。

過ぎた日程は巡回のたびに消える。画面側でも表示のたびに今日で足切りしているので、
巡回が止まっても終わった日程が残ることはない。

**サイトに到達できなかった学校は、前回の値をそのまま残す。**
一時的な通信エラーで全校のデータが消えるのを防ぐため。
...
- **私立4校の入学金が未取得**。連合会のPDFに載っていない学校。
- **写真が無い22校**。Wikipediaの記事に画像が載っていない学校。
- **住所と座標が食い違う3校**（阪南、成美、近畿大学泉州）。画面に警告が出る。
- **定期代の金額は出していない。** 鉄道運賃に機械で読める公開データが無いため、
  「定期代」ボタンから Yahoo!路線情報 の検索を開く形にしている（CLAUDE.md に経緯）。
- **阪堺電気軌道・バス路線が未収録。**
- **オープンスクールの日程は全校ぶんは取れない。** 画像だけのお知らせ、PDF、
  JavaScript で描画するページからは読み取れない。申込フォームのリンクも、
  トップから3ページ以内に無ければ見つからない。
